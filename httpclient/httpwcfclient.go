...
func HelloMethodFunction(helloChan <-chan int) {
	for {
		...
		resp, err := http.Get("http://senthilkumar:8080/CompareService/CompareService.svc/hello")
		if err != nil {
			fmt.Println(err.Error())
		}
		resp.Body.Close()
...
func PyramidConstructorFunction(pyramidChan <-chan int) {
	for {
		_ = <-pyramidChan
		requestStart := time.Now()
		resp, _ := http.Get("http://senthilkumar:8080/CompareService/CompareService.svc/StarPyramidGenerator/15")
		resp.Body.Close()
		requestEnd := time.Now()
...
func HelloJsonFunction(JsonChan <-chan int) {
	for {
		_ = <-JsonChan
		requestStart := time.Now()
		req, err := http.NewRequest("POST", "http://senthilkumar:8080/CompareService/CompareService.svc/helloJson", bytes.NewBuffer(jsonReq))
		if err != nil {
			fmt.Println(err.Error())
		}
...
func HelloDbFunction(helloDbChan <-chan int) {
	for {
		_ = <-helloDbChan
		requestStart := time.Now()
		req, err := http.NewRequest("POST", "http://senthilkumar:8080/CompareService/CompareService.svc/helloDb", bytes.NewBuffer(jsonReq))
		if err != nil {
			fmt.Println(err.Error())
		}
		req.Header.Add("User-Agent", "myClient")
...
func CreateCandidateFunction(CreateCandChan <-chan int) {
	for {
		_ = <-CreateCandChan
		requestStart := time.Now()
		req, err := http.NewRequest("POST", "http://senthilkumar:8080/CompareService/CompareService.svc/CreateCandidate", bytes.NewBuffer(candidateReq))
		if err != nil {
			fmt.Println(err.Error())
		}
		req.Header.Add("User-Agent", "myClient")
